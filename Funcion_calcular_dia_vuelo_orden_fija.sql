...
alter FUNCTION [dbo].[calcular_dia_vuelo_orden_fija] (@fecha datetime, @idc_farm nvarchar(2))
RETURNS datetime 
...
BEGIN
	...
	set @fecha = @fecha - @dias_atras_finca - @dias_restados_despacho_distribuidora

	--select @dia_semana = datepart(dw, @fecha)
	select @dia_semana = datepart(dw, @fecha-1)
...
--			when datepart(dw,@fecha_inicial) < id_dia_despacho then @fecha_inicial-(datepart(dw,@fecha_inicial) - id_dia_despacho + 7)
			...
			when datepart(dw,@fecha_inicial-1) = id_dia_despacho then @fecha_inicial - 7
			when datepart(dw,@fecha_inicial-1) > id_dia_despacho then @fecha_inicial-(datepart(dw, @fecha_inicial-1) - id_dia_despacho)
			when datepart(dw,@fecha_inicial-1) < id_dia_despacho then @fecha_inicial-(datepart(dw,@fecha_inicial-1) - id_dia_despacho + 7)
		end
		from @temp 
		where id_dia_despacho = @dia_semana
	end
