/****** Object:  StoredProcedure [dbo].[inv_insertar_pieza_postcosecha]    Script Date: 10/06/2007 13:56:13 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[inv_insertar_pieza_postcosecha] 
...
IF (len(@idc_grado_flor)>2 OR len(@idc_variedad_flor)>2 OR len(@idc_tipo_flor)>2)
	BEGIN
		...
		values ('el tamaño de las variables de flor es mayor a 2'+space(2)+'dataerror:'+space(1)+
				...
				'tipo:'+space(1)+@idc_tipo_flor+','+space(1)+'unidades por pieza:'+space(1)+convert(nvarchar, @unidades_por_pieza)+','+space(1)+
...
ELSE
	...
	(id_grado_flor, id_variedad_flor, unidades_por_pieza, fecha_entrada)
	select gf.id_grado_flor, vf.id_variedad_flor, @unidades_por_pieza, (CAST(CONVERT(char(12),@fecha,113)+(LEFT(@hora, 2) 
