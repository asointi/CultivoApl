...
BEGIN
	...
	id_variedad_flor,
	id_grado_flor,
	floor(sum(cantidad_unidades_inventario_total)) as cantidad_unidades_inventario_total into #inventario
	from #temp 
	where tipo_orden = 1
	group by
	id_farm,
	...
	id_variedad_flor,
	id_grado_flor,
	floor(sum(cantidad_unidades_prebook_total)) as cantidad_unidades_prebook_total into #prebook
	from #temp 
	where tipo_orden = 2
	group by
	id_farm,
	...
	update #inventario
	set cantidad_unidades_inventario_total = #inventario.cantidad_unidades_inventario_total - #prebook.cantidad_unidades_prebook_total
	from #inventario,
	#prebook
	where #prebook.id_farm = #inventario.id_farm
	and #prebook.id_variedad_flor = #inventario.id_variedad_flor
	and #prebook.id_grado_flor = #inventario.id_grado_flor

	update #temp
	set inventario = floor(#inventario.cantidad_unidades_inventario_total / #temp.unidades_por_pieza)
	from #temp, #inventario
	...
	update #temp
	set prebook = floor(#prebook.cantidad_unidades_prebook_total / #temp.unidades_por_pieza),
	cantidad_unidades_prebook_total = #prebook.cantidad_unidades_prebook_total
	from #temp, #prebook
	where #temp.id_farm = #prebook.id_farm
	and #temp.id_variedad_flor = #prebook.id_variedad_flor
	and #temp.id_grado_flor = #prebook.id_grado_flor
	and tipo_orden = 2
	...
	update #temp
	set prebook = 0
	where prebook is null

	update #temp
	set saldo = inventario
