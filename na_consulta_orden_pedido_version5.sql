...
create PROCEDURE [dbo].[na_consulta_orden_pedido_version5]
...
) as valor_pactado_cultivo into #temp
from orden_pedido, 
tipo_factura, 
tipo_flor, 
variedad_flor, 
...
begin
	if(@idc_cliente_inicial = '' and  @idc_cliente_final = '' and @fecha_inicial = '' and @fecha_final = '' and @idc_farm_inicial = '' and @idc_farm_final = '')
	begin
		select *
		from #temp
	end
	else
	if(@fecha_inicial <> '' and @idc_cliente_inicial = '' and  @idc_cliente_final = '' and @idc_farm_inicial = '')
	begin
		select *
		from #temp
		where 
	...
	if(@idc_cliente_final <> '' and @fecha_inicial = '' and @idc_farm_inicial = '')
	begin
		select * 
		from #temp
	...
	if(@idc_farm_inicial <> '' and @fecha_inicial = '' and @idc_cliente_final = '')
	begin
		select * 
		from #temp
		where ltrim(rtrim(idc_farm)) > =
		CASE
			WHEN ltrim(rtrim(@idc_farm_inicial)) = '' THEN '%%'
			else ltrim(rtrim(@idc_farm_inicial))
		END
		and ltrim(rtrim(idc_farm)) < = ltrim(rtrim(@idc_farm_final))
	end
	else
	if(@idc_farm_inicial <> '' and @fecha_inicial <> '' and @idc_cliente_final = '')
	begin
	...
	begin
		...
		END
		and ltrim(rtrim(idc_farm)) < = ltrim(rtrim(@idc_farm_final))
		and ltrim(rtrim(idc_cliente_despacho)) > =
		CASE
			WHEN ltrim(rtrim(@idc_cliente_inicial)) = '' THEN '%%'
			else ltrim(rtrim(@idc_cliente_inicial))
		END
