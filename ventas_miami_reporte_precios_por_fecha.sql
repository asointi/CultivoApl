...
alter PROCEDURE [dbo].[ventas_miami_reporte_precios_por_fecha] 
...
set @fecha_inicial = CONVERT(VARCHAR(25),DATEADD(dd,-(DAY(DATEADD(mm,-1,getdate()))-1),DATEADD(mm,-1,getdate())),103)
...
and credito.fecha_numero_credito > = @fecha_inicial 
...
insert into bd_cultivo.bd_cultivo.dbo.detalle_facturacion_dia_por_fecha (comercializadora, fecha, fulles, unidades, valor)
...
from #resultado
where fecha_factura < convert(datetime, convert(nvarchar, getdate(), 103))
group by fecha_factura
union all
select @comercializadora as comercializadora,
detalle_facturacion_dia_por_fecha.fecha,
detalle_facturacion_dia_por_fecha.fulles,
detalle_facturacion_dia_por_fecha.unidades_vendidas,
detalle_facturacion_dia_por_fecha.valor
from facturacion_dia,
detalle_facturacion_dia_por_fecha
where facturacion_dia.id_facturacion_dia = detalle_facturacion_dia_por_fecha.id_facturacion_dia
and facturacion_dia.fecha_facturacion_dia = convert(datetime, convert(nvarchar, getdate(), 103))
