...
alter PROCEDURE [dbo].[na_editar_bloque_reporte_areas]
...
if(@accion = 'rango_periodo')
begin
	set @conteo = 1

	while (@numero_periodos > @conteo)
	begin
		insert into #temp
		(
		id_bloque,
		...
		max(pieza_postcosecha.fecha_entrada) as fecha_maxima,
		sum(pieza_postcosecha.unidades_por_pieza) as cantidad_tallos,
		datepart(yy, dateadd(yyyy, -@conteo, @fecha_inicial1)) as ano,
		convert(nvarchar, dateadd(yyyy, -@conteo, @fecha_inicial1), 101) + ' - ' + convert(nvarchar, dateadd(yyyy, -@conteo, @fecha_final1), 101),
		dateadd(yyyy, -@conteo, @fecha_inicial1),
		dateadd(yyyy, -@conteo, @fecha_final1)
		from bloque left join tipo_bloque on bloque.id_tipo_bloque = tipo_bloque.id_tipo_bloque,
		pieza_postcosecha,
		tipo_flor,
		variedad_flor
		...
		and variedad_flor.id_variedad_flor = @id_variedad_flor
		and convert(datetime,convert(nvarchar,pieza_postcosecha.fecha_entrada,101)) between
		dateadd(yyyy, -@conteo, @fecha_inicial1) and dateadd(yyyy, -@conteo, @fecha_final1)
		and convert(datetime,convert(nvarchar,pieza_postcosecha.fecha_entrada,101)) <> convert(datetime, convert(nvarchar, getdate(), 101))
		and exists 
		(
			select #siembra.id_bloque 
...
from #temp
order by convert(int, ano) desc,
convert(int, periodo) desc,
convert(int, promedio) desc
