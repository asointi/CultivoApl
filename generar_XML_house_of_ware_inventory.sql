USE [BD_Fresca]
GO
/****** Object:  StoredProcedure [dbo].[generar_XML_house_of_ware_inventory]    Script Date: 9/20/2013 10:04:17 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[generar_XML_house_of_ware_inventory]
...
@valor_fuel_por_caja_full decimal(20,4),
@porcentaje_adicional_por_caja_full decimal(20,4)
...
select @valor_fuel_por_caja_full = valor_fuel_por_caja_full from configuracion_bd
select @porcentaje_adicional_por_caja_full = porcentaje_adicional_por_caja_full from configuracion_bd
/***************************************************************/
...
sum(tipo_caja.factor_a_full) as factor_a_full,
ltrim(rtrim(pais.nombre_pais)) as nombre_pais into #inventario
from pieza, 
tipo_flor,
variedad_flor,
grado_flor,
...
id_grado_flor, 
code,
max(valor_producto.id_valor_producto) as id_valor_producto into #precio
from valor_producto 
...
alter table #inventario
add Fuel decimal(20,2)

update #inventario
set Fuel = @valor_fuel_por_caja_full * tipo_caja.factor_a_full
from caja, tipo_caja
...
select 
replace(nombre_tipo_caja + space(1) + ltrim(rtrim(nombre_tipo_flor)) + space(1) + ltrim(rtrim(nombre_variedad_flor)) + space(1) + ltrim(rtrim(nombre_color)) + space(1) + ltrim(rtrim(nombre_grado_flor)) + space(1) + convert(nvarchar,unidades_por_pieza), '&', 'and') as flower,
nombre_tipo_caja as boxsize,
replace(ltrim(rtrim(nombre_tipo_flor)), '&', 'and') as flowertype,
replace(ltrim(rtrim(nombre_variedad_flor)), '&', 'and') as variety,
replace(ltrim(rtrim(nombre_color)), '&', 'and') as color,
replace(ltrim(rtrim(nombre_grado_flor)), '&', 'and') as grade,
unidades_por_pieza as packing,
convert(decimal(9,3),precio) as price,
replace(nombre_tipo_caja + space(1) + ltrim(rtrim(nombre_tipo_flor)) + space(1) + ltrim(rtrim(nombre_variedad_flor)) + space(1) + ltrim(rtrim(nombre_grado_flor)), '&', 'and') as name,
0 as FedexRate,
sum(piezas_total) as QOH,
fuel,
convert(decimal(9,3), 0) as additional_fees_per_box,
nombre_pais into #floralship
from #inventario
group by 
idc_color,
idc_tipo_flor,
...
unidades_por_pieza,
convert(decimal(9,3),precio),
precio,
fuel,
nombre_pais

/*restringir el inventario con el tope maximo definido por el usuario*/
update #floralship
set QOH = configuracion_bd.inv_tope_maximo_inventario
from #floralship, configuracion_bd
where  QOH > configuracion_bd.inv_tope_maximo_inventario

/*enviar resultados de inventario a pantalla*/
select * from #floralship

/*eliminacion de tablas temporales*/
drop table #floralship
