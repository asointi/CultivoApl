...
alter PROCEDURE [dbo].[generar_XML_IFD]
...
'Colombia' as origin,
'FRESCA' as vendno,
idc_tipo_flor+idc_variedad_flor+idc_color+idc_grado_flor as vpartno,
'' as item,
ltrim(rtrim(nombre_tipo_flor))+' '+ltrim(rtrim(replace(nombre_variedad_flor, '&', 'AND')))+' '+ltrim(rtrim(nombre_color))+' '+ltrim(rtrim(nombre_grado_flor)) as descrip,
left(CONVERT(NVARCHAR,GETDATE(),120),10) AS availdate,
left(CONVERT(NVARCHAR,GETDATE()+8,120),10) as availdateto,
'' as brand,
'' as memo,
'' as fob,
sum(piezas_disponibles) * unidades_por_pieza as onhand,
convert(decimal(9,3),precio) as cost,
unidades_por_pieza as qtyxbox,
sum(piezas_disponibles) as boxes,
unidades_por_pieza as bunchsize,
0 as bunit,
'stem' as packtype,
convert(decimal(20,4),(sum(factor_a_full)/sum(piezas_disponibles))) as FBE,
nombre_abreviado_tipo_caja as afbe,
'' as segno,
idc_tipo_flor as category,
ltrim(rtrim(idc_tipo_flor)) as type,
ltrim(rtrim(replace(nombre_variedad_flor, '&', 'AND'))) as variety,
ltrim(rtrim(idc_color)) as color,
...
--update #IFD
--set csrec = configuracion_bd.inv_tope_maximo_inventario
--from configuracion_bd
--where  csrec > configuracion_bd.inv_tope_maximo_inventario
